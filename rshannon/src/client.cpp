...
* @Last Modified time: 2016-02-08 17:06:23
*/
...
void Client::process_command(string cmd) {
    ...
    if(logged_in) {
		...
		// NOT logged into a server. Note that EXIT, and AUTHOR
		// are allowed to be processed regardless of whether the client
		...
		if (operation == EXIT) {
			...
			notify_error(operation, "You must be logged into a server to run this command.");
		}
	}
}
